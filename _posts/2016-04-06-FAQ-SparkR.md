...
Assuming that the installation directory was `/opt/` directory one can run SparkR package in shell with
...
SparkR with the non-default directory, so you should get familiar with `lib.loc` parameterin `library` function.
...
Spark provide greate machine learning library called [MLlib](http://spark.apache.org/mllib/) with alghoritms based on stochastic gradient
...
## Q13: Where can I read more?! I want more examples!
